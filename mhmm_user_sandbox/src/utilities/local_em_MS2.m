function local_em_outputs = local_em_MS2 (fluo_values, v, noise, pi0_log, ...
                                     A_log, K, w, alpha, n_steps_max, eps)
    ...
    %             convergence
    % logL_tot: log-likelihood at each forward-backward iteration
    % max(logL_tot): log-likelihood at convergence
    ...
    for i_tr = 1:n_traces
        ...
        for t = 1 : (w-1)
            ...
            temp_var(:,t) = log(abs(exp(temp_var(:,t)) - count_reduction));
        end
    ...
    local_em_outputs = struct('pi0_log', pi0_log, 'A_log', A_log, ...
        ...
        'max_bw', baum_welch, 'n_iters', iter, 'total_time', run_time);
