function local_em_outputs = local_em_MS2_reduced_memory_truncated_ss_no_emission(fluo_values, ...
              v, noise, A_log, K, w, kappa, n_steps_max, eps)
...
%     logD = log(D);
    ...
    % Structure to store alpha, beta, p_s_log and p_ss_log matrices
    soft_struct = struct;
    
    alpha_log_soft = cell([1, n_traces]);
    beta_log_soft = cell([1, n_traces]);
    p_z_log_soft = cell([1, n_traces]);
    p_zz_log_soft = cell([1, n_traces]);
...
%             p_s0_log(i) = pi0_log(naive_states(w));
        ...
        for i_tr = 1:n_traces
            ...
            if (one_more == 1 || baum_welch == n_steps_max)
                alpha_log_soft_singleTrace = zeros(K,fluo_lengths{i_tr});
                beta_log_soft_singleTrace = zeros(K,fluo_lengths{i_tr});
                p_z_log_soft_singleTrace = zeros(K,fluo_lengths{i_tr});
                for k = 1:K
                    ind = find(digit_first_list == k);
                    for t = 1:fluo_lengths{i_tr}
                        alpha_log_soft_singleTrace(k,t) = ...
                            log_sum_exp_positive(alpha_matrix_log(ind,t));
                        beta_log_soft_singleTrace(k,t) = ...
                            log_sum_exp_positive(beta_matrix_log(ind,t));
                        p_z_log_soft_singleTrace(k,t) = ...
                            log_sum_exp_positive(p_s_log(ind,t));
                    end
                end
                alpha_log_soft{i_tr} = alpha_log_soft_singleTrace;
                beta_log_soft{i_tr} = beta_log_soft_singleTrace;
                p_z_log_soft{i_tr} = p_z_log_soft_singleTrace;
                
                p_zz_log_soft_singleTrace = zeros(K,K,fluo_lengths{i_tr}-1);
                for m = 1:K
                    for n = 1:K
                        for t = 1 : (fluo_lengths{i_tr}-1)
                            A_ind = (1 + (t-1)*K^(w-1)) : t*K^(w-1);
                            ind = A_log_maximization_ind{m,n}(A_ind);
                            p_zz_log_soft_singleTrace(m,n,t) = ...
                                log_sum_exp_positive(p_ss_log(ind));
                        end
                    end
                end
                p_zz_log_soft{i_tr} = p_zz_log_soft_singleTrace;
            end
        end
        ...
        if (max([A_norm_rel_change, logL_norm_change]) < eps && one_more == 0)
            logL_tot = logL_tot(1:baum_welch);
            one_more = 1;
        end
    end
    
    % add the "soft" variables into a structure
    soft_struct.alpha_log_soft = alpha_log_soft;
    soft_struct.beta_log_soft = beta_log_soft;
    soft_struct.p_z_log_soft = p_z_log_soft;
    soft_struct.p_zz_log_soft = p_zz_log_soft;
    
    % -------------- collection of outputs into a cell -----------------
    local_em_outputs = struct('A_log', A_log, ...
